...
function Destination(props) {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            setLoading(true);
            //  console.log(filter);
            let result = await destinationApi.getAll(filter);
    ...
    const handleDeleteMany=()=>{
        Modal.confirm({
            ...
            onOk: async () => {
                ...
                // console.log(ids);
                 await destinationApi.deleteMany(ids);
                setDeleting(true);
                message.success({ content: intl.formatMessage({ id: "global.deleted_success" }) });
            },
            onCancel() { },
    ...
    const onChangeStatus = async (ischecked, record) => {
        //   setLoading(true)
        await destinationApi.update({ state: ischecked, id: record.id });
        //  setLoading(false);
        onReload();
    ...
    return (
        ...
        <Card bordered={false} style={{ minHeight: '100%' }}>
            ...
            />
            {modal ?
                <DestinationForm open={modal} onClose={onCloseModal} currentData={item} onReload={onReload} />
                : null}
